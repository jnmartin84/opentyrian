...
void JE_darkenBackground(JE_word neat)  /* wild detail level */
{
	...
	for (y = 184; y; y--)
	{
		for (x = 264; x; x--)
		{
			*s = ((((*s & 0x0f) << 4) - (*s & 0x0f) + ((((x - neat - y) >> 2) + *(s-2) + (y == 184 ? 0 : *(s-(screenpitch-1)))) & 0x0f)) >> 4) | (*s & 0xf0);
			s++;
		}
		s += screenpitch - 264;
	}
...
void draw_background_1(uint8_t *surface)
{
	//printf("\n\ndraw_background_1\n\n");
//	SDL_FillRect(surface, NULL, 0);
    memset(surface, 0, 320*200);
...
void draw_background_2(uint8_t *surface)
{
...
void draw_background_2_blend(uint8_t *surface)
{
	if (map2YDelayMax > 1 && backMove2 < 2)
		backMove2 = (map2YDelay == 1) ? 1 : 0;
...
void draw_background_3(uint8_t *surface)
{
...
void lava_filter(uint8_t *dst, uint8_t *src)
{
//	assert(src->format->BitsPerPixel == 8 && dst->format->BitsPerPixel == 8);
	...
	const int dst_pitch = screenpitch;//dst->pitch;
	Uint8 *dst_pixel = (Uint8 *)dst + (185 * dst_pitch);
	const Uint8 * const dst_pixel_ll = (Uint8 *)dst;  // lower limit
	
	const int src_pitch = screenpitch;//src->pitch
	const Uint8 *src_pixel = (Uint8 *)src + (185 * src_pitch);
	const Uint8 * const src_pixel_ll = (Uint8 *)src;  // lower limit
...
void water_filter(uint8_t *dst, uint8_t *src)
{
//	assert(src->format->BitsPerPixel == 8 && dst->format->BitsPerPixel == 8);
	...
	const int dst_pitch = screenpitch;
	Uint8 *dst_pixel = (Uint8 *)dst + (185 * dst_pitch);
	
	const Uint8 *src_pixel = (Uint8 *)src + (185 * screenpitch);
	...
	{
		dst_pixel -= (dst_pitch - 320);  // in case pitch is not 320
		src_pixel -= (screenpitch - 320);  // in case pitch is not 320
...
void iced_blur_filter(uint8_t *dst, uint8_t *src)
{
//	assert(src->format->BitsPerPixel == 8 && dst->format->BitsPerPixel == 8);
	
	Uint8 *dst_pixel = dst;
	const Uint8 *src_pixel = src;
	...
	for (int y = 0; y < 184; ++y)
	{
		...
		dst_pixel += (screenpitch - 320);  // in case pitch is not 320
		src_pixel += (screenpitch - 320);  // in case pitch is not 320
	}
}

void blur_filter(uint8_t *dst, uint8_t *src)
{
//	assert(src->format->BitsPerPixel == 8 && dst->format->BitsPerPixel == 8);
...
void initialize_starfield(void)
{
	...
	{
		starfield_stars[i].position = mt_rand() % 320 + mt_rand() % 200 * screenpitch;
		starfield_stars[i].speed = mt_rand() % 3 + 2;
		starfield_stars[i].color = mt_rand() % 16 + STARFIELD_HUE;
	}
}

void update_and_draw_starfield(uint8_t* surface, int move_speed)
{
	...
	for (int i = MAX_STARS-1; i >= 0; --i)
	{
		...
		star->position += (star->speed + move_speed) * screenpitch;

		if (star->position < 177 * screenpitch)
		{
			...
			if (star->color - 4 >= STARFIELD_HUE)
			{
				...
				if (p[star->position + screenpitch] == 0)
					p[star->position + screenpitch] = star->color - 4;

				if (star->position >= screenpitch && p[star->position - screenpitch] == 0)
					p[star->position - screenpitch] = star->color - 4;
			}
		}
	}
